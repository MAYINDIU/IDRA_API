...
async function getPendingPolicies() {
  ...
  const query = `
    ...
    WHERE gender IS NOT NULL AND (flag IS NULL OR UUDATE IS NOT NULL) AND USTATUS IS NULL
...
async function updatePolicyStatus(conn, row, status, message, code) {
  ...
  const query = hasUUDATE
    ? `UPDATE idraump.${table}
       SET flag='1', ustatus='1', udate=SYSDATE, code=:code, msg=:message, status=:status
       WHERE policynumber=:policyNumber AND (flag IS NULL OR ustatus IS NULL) AND UUDATE IS NOT NULL`
    : `UPDATE idraump.${table}
